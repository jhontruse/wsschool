...
public class MenuRepository implements IMenuRepository {
    ...
    private static final String getRolByUsuarioSQL = " SELECT M.* FROM MENU M INNER JOIN ROL_MENU RM ON M.ID_MENU = RM.ID_MENU INNER JOIN ROL R ON R.ID_ROL = RM.ID_ROL INNER JOIN USUARIO_ROL UR ON UR.ID_ROL = R.ID_ROL INNER JOIN USUARIO U ON U.ID_USUARIO = UR.ID_USUARIO WHERE U.USUARIO = ? ";

    private static final String getRolAllSQL = " SELECT * FROM MENU ";
